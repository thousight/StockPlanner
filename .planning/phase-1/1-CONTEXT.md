...
- **Persona-Driven:** Each agent has a dedicated `prompts.py` and `agent.py` in its own directory, defining a specific "persona" to guide its reasoning.
...
- **Direct Communication:** Agents can communicate directly (e.g., Analyst -> Research for more data) while maintaining their own internal "stop" logic.

## 2. Agent Findings & Data Handling

- **Format:** Agents return findings in Natural Language, optimized for direct inclusion in the final report.
- **Conflict Resolution:** Data conflicts (e.g., differing price targets) must be resolved by the agent agent before reporting.
...
- **Layout:** Expandable rows (`st.status`) for each agent to manage parallel execution visualization.

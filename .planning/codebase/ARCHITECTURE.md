...
**Key Characteristics:**
- **Stateful Agent Workflow:** Uses LangGraph to manage a stateful, directed acyclic graph (DAG) of specialized agents.
- **Service-Oriented Agents:** Each agent in the graph has a single responsibility (research, analysis, maintenance).
...
- Depends on: `src/state.py`, `src/agents/`
- Used by: `src/app.py`
...
- Depends on: `src/state.py`, `langchain`, `src/database/`
- Used by: `src/graph.py`
...
- Used by: `src/app.py`, `src/agents/`

## Data Flow

**Portfolio Analysis Flow:**

1. **Trigger:** User clicks "Run Portfolio Analysis" in the Streamlit UI (`src/app.py`).
...
3. **Graph Execution:** The UI invokes the compiled LangGraph (`src/graph.py`).
4. **Research Phase:** The `research_agent` (`src/agents/research/agent.py`) fetches market news and stock data in parallel using `ThreadPoolExecutor`.
5. **Analysis Phase:** The `analyst_agent` (`src/agents/analyst/agent.py`) formats the gathered data and uses `ChatOpenAI` (GPT-4o) to generate an investment report.
6. **Maintenance Phase:** The `cache_maintenance_agent` (`src/agents/agents.py`) performs cleanup tasks like removing expired news entries.
7. **Result Delivery:** The final state is returned to the UI, which displays the generated `analysis_report`.

**State Management:**
- Handled by `AgentState` (`src/state.py`), a `TypedDict` that stores messages, portfolio data, research results, and the final analysis report.

## Key Abstractions

**AgentState:**
- Purpose: The shared memory/context passed between agents in the graph.
...
**Patterns:**
- **Node-level Try-Except:** Agents like `analyst_agent` catch LLM exceptions and return an error message in the state rather than crashing the graph.
- **DB Session Management:** `get_db` generator in `src/database/database.py` ensures sessions are closed even if errors occur.

## Cross-Cutting Concerns

**Logging:** Standard output prints for debugging (e.g., in `analyst_agent`).
